...
* **Resumen:** Simulación de un flujo de comunicación lineal familiar (Madre - Padre - Hijo). El proceso `Madre` emite una instrucción por la salida estándar, capturada mediante un Pipe (`|`) de la terminal por el proceso `Padre`. Este último orquesta el ciclo de vida de un subproceso dinámico (`Hijo`) a través de `ProcessBuilder`, transmitiéndole el flujo de datos y renderizando la respuesta final de forma coordinada.
